...
import GrantsList, { type Grant } from "@/components/GrantsList";
import grants from "@/data/grants.json";

export const metadata: Metadata = { title: "Grants" };

type Record = { id: string; title: string; duration: string | null; funder: string };
...
export default function GrantsPage() {
  // Whether a grant is still running is decided here, at build time, since the
  // site is a static export and there is no request to decide it at. A grant
  // ending this year keeps saying Active until the next deploy — accurate to
  // within one deploy, and this site deploys on every push. It is settled on
  // the server rather than in the browser so every visitor sees the same page.
  ...
  const items: Grant[] = (grants as Record[])
    .map((g) => ({ ...g, active: years(g.duration).to >= thisYear }))
    .sort((a, b) => {
